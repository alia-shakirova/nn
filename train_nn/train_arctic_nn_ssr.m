...
vars = {'ssr';'tisr';'tciw';'tclw';'tcwv';'hcc';'mcc';'lcc';'sp';'tco3';'fal'};
time = ncread([data_path,'SSR_era5.nc'],'time');
lon = ncread([data_path,'SSR_era5.nc'],'longitude');
lat = ncread([data_path,'SSR_era5.nc'],'latitude');
maxlat = 31;
lat = lat(1:maxlat);
y1 = 1; y2 = 336; %217
time1 = time(y1:y2);
ssr = ncread([data_path,'SSR_era5.nc'],'ssr');
ssr = ssr(:,1:maxlat,y1:y2);
ssr = ssr/24/3600;
numlon = size(lon,1);
numlat = size(lat,1);
numt = size(time1,1);
...
%save NN model
save('Net_ssr_arctic.mat','net');
save 'Net_ssr_arctic.mat' max_x min_x -append
toc
